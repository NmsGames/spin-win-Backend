...
async function PreviousWinData(RoundCount){
    try{
        ...
        let sql =  `SELECT * FROM (SELECT * FROM current_round ORDER by cr_id DESC LIMIT 1,10) sub ORDER BY cr_id ASC`
...
const JoinGame = async(req,CurrRoundCount,gameState) => {
	...
	try {
		...
		let sql = 'SELECT count(*) as cnt FROM `current_round` WHERE date(created) =?'; 
	    let checkRound = await db.query(sql,[current_date]); 

		// // Check already bet confirmed
		const roundDate = moment(new Date()).format("YYYYMMDD")+1;
...
async function setRoundCount(){
    ...
    try {
		let current_date = moment(new Date()).format("YYYY-MM-DD");
		let sql1  = 'SELECT count(*) as cnt FROM `current_round` WHERE date(created) =?';  
    	let WinNo = GetRandomNo(0,11);
    	let WinX  = '1x'; 
		...
		const date = moment(new Date()).format("YYYYMMDD")+1; 
		const tRound = (parseInt(date)+parseInt((checkRound[0].cnt)))
		let sql5 = `SELECT * FROM current_round ORDER by cr_id DESC LIMIT 0,1`;
	    let roundData = await db.query(sql5); 
		let sql2 = `UPDATE current_round SET round_count=? WHERE cr_id = ?`;
...
async function calcWinningNo(RoundCount){
	...
	let data = {}
    try {
	    let sql = `SELECT * FROM current_round ORDER by cr_id DESC LIMIT 0,1`;
...
async function calcWinningPoints(){
	try {
		let sql4 = `SELECT * FROM current_round ORDER by cr_id DESC LIMIT 1,1`;
		...
		let players = await db.query(sql,[3,RoundCount,1]);
		let winning_rate1 = 3
		...
		if(players.length>0){  
			...
			{
				let win_points =  0;
				let {no_0,no_1,no_2,no_3,no_4,no_5,no_6,no_7,no_8,no_9,no_10,no_11,nof0,nof1,nof2,noaf0,noaf1,noaf2,noaf3,player_id,id} = bet;
